...
func _ready():
	# ---- TEST ----
	$TestContainer/Icon.texture = $GameItem.sprite
	$TestContainer/Name.text = $GameItem.item_name
	$TestContainer/Count.text = str($GameItem.count)
	$TestContainer/IsKey.text = str($GameItem.is_key_item)
